...
// CHECK-SYSTEM: mul2[0:0](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:1](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:2](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:3](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:4](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:5](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:6](2 * 4 = 8)
// CHECK-SYSTEM: mul2[0:7](2 * 4 = 8)
// arith.addf 8 + 4 = 12
...
module @example {
  ...
  stream.executable private @executable {
    stream.executable.export public @simple_mul workgroups(%workload: index) -> (index, index, index) {
      ...
      %x = affine.apply affine_map<()[s0] -> (s0 ceildiv 4)>()[%workload]
    ...
    builtin.module {
      ...
      func.func @simple_mul(
        ...
        // workgroup size of 4x1x1 (which is exceedingly small for CPUs but
        // useful for demonstration).
        %workgroup_id_x = flow.dispatch.workgroup.id[0] : index
        %tid = affine.apply affine_map<()[s0] -> (s0 * 4)>()[%workgroup_id_x]
        ...
        %extra_offset0 = arith.muli %stride0, %tid : index
        %extra_offset1 = arith.muli %stride1, %tid : index
        %extra_offset2 = arith.muli %stride2, %tid : index

        %offset0_workgroup = arith.addi %offset0, %extra_offset0 : index
        %offset1_workgroup = arith.addi %offset1, %extra_offset1 : index
        %offset2_workgroup = arith.addi %offset2, %extra_offset2 : index

        // TODO(ramiro050): fix this calculation
        %dim_minus_tid = arith.subi %dim, %tid : index
        %dim_minus_tid_minus_1 = arith.subi %dim_minus_tid, %c1 : index
        %dim_workgroup = arith.subi %dim, %tid : index
        ...
        func.call @simple_mul_workgroup(
            %base0, %offset0_workgroup, %base1, %offset1_workgroup, %base2, %offset2_workgroup, %dim_workgroup, %workgroup_id_x)
