...
// CHECK-LABEL:   func.func @fully_connected$vecmat(
// CHECK-SAME:                           %[[LHS:.*]]: tensor<1x100xi8>,
// CHECK-SAME:                           %[[RHS:.*]]: tensor<200x100xi4>) -> tensor<1x200xf32> {
// CHECK:           %[[C8:.*]] = stablehlo.constant dense<8>
// CHECK:           %[[C8_I4:.*]] = stablehlo.convert %[[C8]] : (tensor<{{.*}}i8>) -> tensor<{{.*}}i4>
// CHECK:           %[[RHS_UI4:.*]] = stablehlo.xor %[[RHS]], %[[C8_I4]]
// CHECK:           %{{.*}} = xnnpack.fully_connected_nc_qd8_f32_qc4w_vecmat %[[LHS]], %[[RHS_UI4]] transpose_rhs = false : (tensor<1x100xi8>, tensor<200x100xi4>) -> tensor<1x200xf32>
func.func @fully_connected$vecmat(%input : tensor<1x100xi8>, %kernel : tensor<200x100xi4>) -> tensor<1x200xf32> {
