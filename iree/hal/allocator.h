...
// properties by matching the first heap that meets the requirements. When
// enumerated the heaps will be in preferred usage order such that the matching
// will always select the most preferred memory heap first (usually the
// fastest). If no heaps can satisfy a given request then allocation will fail.
typedef struct iree_hal_allocator_memory_heap_t {
  // Indicates the memory type bits that describe the memory behavior.
  iree_hal_memory_type_t type;

  // Indicates what kind of usage is allowed of buffers allocated with this
  // type. For example, device local memory may not allow mapping while cached
  // host local memory may not allow usage in dispatches.
...
// Returns an index in |heaps| that can satisfy the given buffer |params|.
// Heaps are searched in order and the first that can meet all the requirements
// will be selected.
// Returns -1 if no heap is available.
